...
export async function POST(request: NextRequest) {
  ...
  if (isRateLimited(ip)) {
    return NextResponse.json(
      {
        error: 'rate_limited',
        message: "You've reached the question limit for this hour. Check back soon, or message Christian directly at christian@bourlier.ai.",
      },
      { status: 429 }
    );
  }
  ...
  try {
    ...
    const fallbackMessage = "I'm sorry, I'm having trouble answering that right now. Please try again in a moment, or reach out to Christian directly at christian@bourlier.ai.";
    recordRequest(ip);
